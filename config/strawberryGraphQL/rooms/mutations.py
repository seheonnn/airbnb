...
def add_room(info: Info, category_pk: int, amenities: typing.List[int], name: str, country: str, city: str, price: int, rooms: int, toilets: int, description: str, address: str, pet_friendly: bool, kind: RoomKindChoices):
    ...
    try:
        with transaction.atomic():
            room = Room.objects.create(name=name, country=country, city=city, price=price, rooms=rooms, toilets=toilets, description=description, pet_friendly=pet_friendly, kind=kind, owner=info.context.request.user, category=category)
